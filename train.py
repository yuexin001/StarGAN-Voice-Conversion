    ...
    for epoch in range(1, EPOCH + 1, 1):
        ...
        for i in range(num_samples // BATCHSIZE):
            ...
            if num_iterations > 100000:
                domain_classifier_learning_rate = max(0,
                                                      domain_classifier_learning_rate - domain_classifier_learning_rate_decay)
            ...
            for one_filename, one_name, one_target in zip(files_shuffled[start:end], names_shuffled[start:end],
                                                          pre_targets):
                t = os.path.normpath(one_target).rsplit(os.sep, maxsplit=1)[1]
            ...
            generator_loss, discriminator_loss, domain_classifier_loss = model.train(input_source=X, input_target=X_t,
                                                                                     source_label=y, target_label=y_t,
            ...
            if num_iterations % 10 == 0:
                print(
                    'Iteration: {:07d},Generator Loss : {:.3f}, Discriminator Loss : {:.3f}, domain_classifier_loss: {:.3f}' \
        ...
        print('Time Elapsed for Epoch %d: %02d:%02d:%02d' % (
            epoch, time_elapsed_epoch // 3600, (time_elapsed_epoch % 3600 // 60),
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='StarGAN Voice conversion model.')

    parser.add_argument('--processed_dir', type=str, default=processed_dir)
    parser.add_argument('--test_wav_dir', type=str, default=test_wav_dir)
    ...
    print('Time: %02d:%02d:%02d' % (time_elapsed // 3600, (time_elapsed % 3600 // 60), (time_elapsed % 60 // 1)))
