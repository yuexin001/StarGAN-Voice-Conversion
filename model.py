...
class StarGANVC(object):
    ...
    def build_model(self):
        ...
        self.target_label_reshaped = tf.placeholder(tf.float32, [None, 1, 1, SPEAKERS_NUM],
        ...
        self.generated_back = self.generator(self.generated_forward, self.source_label, reuse=True,
                                             scope_name='generator')

        # Domain classify loss
        self.domain_out_real = self.classifier(self.target_real, reuse=False, scope_name='classifier')
        self.domain_real_loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.target_label_reshaped, logits=self.domain_out_real))
        ...
        self.discrimination_real = self.discriminator(self.target_real, self.target_label, reuse=False,
                                                      scope_name='discriminator')

        self.discirmination = self.discriminator(self.generated_forward, self.target_label, reuse=True,
                                                 scope_name='discriminator')
        self.generator_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(self.discirmination),
                                                    logits=self.discirmination))
        ...
        self.generator_loss_all = self.generator_loss + self.lambda_cycle * self.cycle_loss + self.lambda_identity * self.identity_loss + self.lambda_classifier * self.domain_real_loss

        # Discriminator loss
        self.discirmination_fake = self.discriminator(self.generated_forward, self.target_label, reuse=True,
                                                      scope_name='discriminator')
        self.discrimination_real_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(self.discrimination_real),
                                                    logits=self.discrimination_real))

        self.discrimination_fake_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(self.discirmination_fake),
                                                    logits=self.discirmination_fake))

        epsilon = tf.random_uniform((self.batchsize, 1, 1, 1), 0.0, 1.0)
        x_hat = epsilon * self.generated_forward + (1.0 - epsilon) * self.input_real

        gradients = tf.gradients(self.discriminator(x_hat, self.target_label, reuse=True, scope_name='discriminator'),
                                 [x_hat])
        _gradient_penalty = 10.0 * tf.square(tf.norm(gradients[0], ord=2) - 1.0)
        self.domain_out_fake = self.classifier(self.generated_forward, reuse=True, scope_name='classifier')
        self.domain_fake_loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.target_label_reshaped, logits=self.domain_out_fake))
        ...
        self.discriminator_optimizer = tf.train.AdamOptimizer(learning_rate=self.discriminator_learning_rate,
                                                              beta1=0.5).minimize(self.discrimator_loss,
        ...
        self.generator_optimizer = tf.train.AdamOptimizer(learning_rate=self.generator_learning_rate,
                                                          beta1=0.5).minimize(self.generator_loss_all,
        ...
        self.generation_test = self.generator(self.input_test, self.target_label_test, reuse=True,
                                              scope_name='generator')

    def train(self, input_source, input_target, source_label, target_label, lambda_cycle=1.0, lambda_identity=1.0,
              lambda_classifier=1.0, generator_learning_rate=0.0001, discriminator_learning_rate=0.0001,
        ...
        generation_f, _, generator_loss, _, generator_summaries = self.sess.run(
            [self.generated_forward, self.generated_back, self.generator_loss, self.generator_optimizer,
             self.generator_summaries],
            feed_dict={self.lambda_cycle: lambda_cycle, self.lambda_identity: lambda_identity,
                       self.lambda_classifier: lambda_classifier, self.input_real: input_source,
                       self.target_real: input_target, self.source_label: source_label, self.target_label: target_label,
                       self.target_label_reshaped: target_label_reshaped,
    ...
    def test(self, inputs, label):
        generation = self.sess.run(self.generation_test,
                                   feed_dict={self.input_test: inputs, self.target_label_test: label})
        return generation
